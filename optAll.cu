...
// start = blockIdx.x * blockDim.x * width + blockIdx.y * blockDim.y;
 
 // __syncthreads();
 __shared__ float  value[BLOCK_SIZE][BLOCK_SIZE];
 
   value[threadIdx.x][threadIdx.y]=in[location]*mul[threadIdx.y];
   __syncthreads();
...
//	b=ty%2;
	 ...
	 int factor= b+b + a - 1;
	 out[location]=in[location]*factor/sum;
